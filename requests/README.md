...
Git-managed Markdown requests record what Commitment wants an external actor,
usually the operator, to provide, approve, configure, purchase, explain, verify,
or otherwise make available. They are not `memory/` (learned observations),
`queue/` (candidate work), human input/inbox, or `runlog.jsonl` (actual events).
Both Commitment and the operator may edit them. No live requests are seeded.

The resource model is open-ended, covering constraints in both the software and
hardware environment and anything else that materially improves usefulness.
Prefer direct observation for platform/resource information; request facts that
cannot be reliably observed. [AGENTS.md](../AGENTS.md#external-resources-and-requests)
describes self-acquisition, examples, spending/identity boundaries, and judgment
about resource impact. Legitimate free self-service within granted capabilities
does not need blanket operator approval. Request only the missing external action
when you cannot reasonably and legitimately acquire the resource yourself.

## Format

Use a descriptive lowercase hyphenated filename, with a normalized title. Before
creating one, scan filenames, normalized titles, and exact origin/source references
in existing requests, including closed ones; update a match. Lowercase the title,
replace punctuation/whitespace runs with `-`, and compare the resulting slug and
references with `rg --files requests` and `rg` searches. No semantic deduplication
or task engine is involved. Cross-link relevant queue/memory entries rather than
copying them. Use this shape; mark irrelevant costs/options as not applicable and
unknown facts as unknown, rather than inventing estimates:

```markdown
---
title: Short resource request
status: requested
requested: YYYY-MM-DD
session: exact COMMITMENT_SESSION_ID
updated: YYYY-MM-DD
origin: observed constraint, queue item, or source reference
related_queue: []
related_memory: []
secret_ref: optional bitwarden:<opaque-id>, NEVER the value
---

## Requested resource / capability / information
What an external actor should make available or do.

## Concrete reason and enabled work
Observed constraint and the useful work this would enable.

## Minimum sufficient requirement
Smallest sufficient capability, information, or action.

## Preferred option and alternatives considered
Preference if any; include self-acquisition attempts/limitations and alternatives.

## Expected value
Why this is worthwhile compared with the fallback.

## Costs and resource impact
Likely monetary cost or evidence-derived target price where relevant; recurring
costs, compute, storage, bandwidth, RAM/VRAM, rate limits, and maintenance.

## Risks and trust boundaries
Permissions, identity, privacy, provider restrictions/licensing, and other risks.

## Evidence / sources
Inspected references supporting the need, options, and cost assumptions.

## Fallback if unavailable
Useful work or a lower-resource alternative that can continue meanwhile.

## Operator notes
Questions, configuration/setup notes, conditions, or decisions.

## Disposition
Reason and date when closed; retain any operator reasoning when deferred/denied.
```
...
| Status | Meaning |
| --- | --- |
| `requested` | External action or information is pending. |
| `granted` | Approved/provided in principle; setup or verification may remain. |
| `denied` | Declined; keep the reason when available. |
| `deferred` | Postponed; retain reason and any conditions for reconsideration. |
| `withdrawn` | Commitment no longer needs it; explain why. |
| `fulfilled` | The requested capability/action is actually available/complete. |

A grant is not proof of fulfillment or unlimited authority. Verify setup before
unblocking dependent work. Silence is not denial. Keep denied, deferred, and
withdrawn requests by default; retain disposition reasoning. Revisit only with
materially new evidence, changed circumstances/prices/alternatives, or an operator
invitation; do not repeatedly nag. Withdraw requests made unnecessary by a better
...
missing SDK/token, wrong permissions, or service failures fail closed with fixed
non-secret errors. Host proxy settings remain available to trusted SDK networking.
...
capability. Uninstall removes the helpers, preserving configuration/credentials
