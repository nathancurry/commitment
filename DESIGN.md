...
The supervisor builds small subprocess environments instead of inheriting ambient `GIT_*` variables. Every Git command names the exact worktree and Git directory, disables replacement objects, pagers, prompts, external diffs, global and system configuration, and fsmonitor, and forces `core.hooksPath` to a supervisor-owned empty directory. Cleanliness commands additionally force case-sensitive paths, executable-bit tracking, uncomposed Unicode path handling, normal stat checks, and non-converting line endings before the Git subcommand; status explicitly reports all non-ignored untracked files. Status is not proof of tracked worktree cleanliness. The supervisor reads each committed blob directly from the pinned `HEAD` object with replacements disabled and compares it to a descriptor-relative, no-follow, bounded worktree read. After reading, it verifies through the already-open parent directory that the pathname remains a regular file naming the opened device and inode. A hard-link alias to that same inode is accepted when descriptor metadata and bytes remain unchanged. Cleanliness requires exact raw-byte, regular-file-type, existence, and executable-bit equality; attributes and clean or smudge conversion do not participate. A same-filesystem probe fails closed when Linux cannot provide case-sensitive names, executable mode bits, or distinct composed and decomposed Unicode names. Replacement refs and configured clean, smudge, or process filters are rejected before worktree-aware inspection. Validated journal bytes are hashed through raw stdin with filters disabled. Index changes use cacheinfo or index-info object IDs only. Explicit author and committer identity does not depend on Git configuration. The repository lock coordinates Commitment runs but cannot lock the filesystem against a same-UID process that bypasses it; hostile mutation after the final pathname-identity check remains out of scope.
...
2. The supervisor rejects replacement refs, resolves the local branch and exact `HEAD`, and records index bytes, index flags, raw tracked worktree state, and non-ignored worktree status. Apply or commit requires the staged index to match pinned `HEAD`, every tracked path to have exact raw bytes, regular-file type, existence, and executable-bit semantics matching pinned `HEAD`, and no non-ignored untracked files. Ignored files and directories are excluded from state comparisons and left untouched.
...
7. The supervisor independently validates the exact rendered bytes and rejects unexpected paths, oversized or malformed data, ignored generated paths, ignored filesystem conflicts at the proposed path or required parents, timeouts, and failed container cleanup.
