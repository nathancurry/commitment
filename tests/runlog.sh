...
expected=$(printf '%s\n' AGENTS.md README.md agent-git.sh commitment-log.sh config.example.env memory/README.md prompt.txt session-outcome.sh tests/git-boundary.sh tests/memory-queue-noop.sh tests/runlog.sh tests/session-regressions.sh tests/terminal-outcome.sh | sort)
