...
Old runlog lines remain untouched and need not match newer outcome fields. The agent never edits this file directly. It records significant events through `commitment-log TYPE "summary" [FIELD=VALUE ...]`, which generates the timestamp, uses the runtime's session ID, JSON-encodes values, and appends one line. Test events require `command` and `result` fields.
